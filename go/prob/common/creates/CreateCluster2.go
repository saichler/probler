...
// CreateCluster2 creates a Kubernetes cluster target that is collected via the
// in-cluster admission-control collector rather than the external kubectl path.
func CreateCluster2(id string) *l8tpollaris.L8PTarget {
	device := &l8tpollaris.L8PTarget{}
	device.TargetId = id
	device.LinksId = common.K8sC_Links_ID
