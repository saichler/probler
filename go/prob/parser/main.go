...
func main() {
	...
	nic.Resources().Services().RegisterServiceHandlerType(&parsing.ParsingServicePoint{})
	nic.Resources().Services().RegisterServiceHandlerType(&poll_config.PollConfigServicePoint{})

	//Register the inventory for NetworkBox as Parser-NetworkBox service name on area 0
	nic.Resources().Services().Activate(parsing.ServicePointType, common2.PARSER_SERVICE_BOX, common2.PARSER_AREA_BOX,
	...
	//as the NetworkBox
	nic.Resources().Services().Activate(parsing.ServicePointType, common2.PARSER_SERVICE_K8s,
	...
	nic.Resources().Services().Activate(poll_config.ServicePointType, poll_config.ServiceName,
